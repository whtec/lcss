...
namespace LCSS.DAL
{
    ...
    public partial class Salary
    {
        ...
        public bool Update(LCSS.Model.Salary model)
        {
            ...
            strSql.Append("Sal_Add_Date=@Sal_Add_Date");
            strSql.Append(" where Sal_ID=@Sal_ID");
            SqlParameter[] parameters = {
					new SqlParameter("@Sal_Year", SqlDbType.Int,4),
					new SqlParameter("@Sal_Month", SqlDbType.Int,4),
            ...
            parameters[2].Value = model.Sal_Add_User;
            parameters[3].Value = model.Sal_Add_Date;
        ...
        public LCSS.Model.Salary GetModel(long Sal_ID)
        {
            ...
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 Sal_ID,Sal_Year,Sal_Month,Sal_Add_User,Sal_Add_Date,Sal_Org_Code from Salary ");
            strSql.Append(" where Sal_ID=@Sal_ID");
            SqlParameter[] parameters = {
					new SqlParameter("@Sal_ID", SqlDbType.BigInt)
			};
        ...
        public LCSS.Model.Salary DataRowToModel(DataRow row)
        {
            ...
            if (row != null)
            {
                ...
                if (row["Sal_Add_Date"] != null && row["Sal_Add_Date"].ToString() != "")
                {
                    model.Sal_Add_Date = DateTime.Parse(row["Sal_Add_Date"].ToString());
                }
                if (row["Sal_Org_Code"] != null)
                {
                    model.Sal_Org_Code = row["Sal_Org_Code"].ToString();
                }
            }
            return model;
        }
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select Sal_ID,Sal_Year,Sal_Month,Sal_Add_User,Sal_Add_Date,Sal_Org_Code ");
        ...
        public DataSet GetList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" Sal_ID,Sal_Year,Sal_Month,Sal_Add_User,Sal_Add_Date,Sal_Org_Code ");
            strSql.Append(" FROM Salary ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
