...
namespace PC.DBUtility
{
    ...
    public abstract class DbHelperSQL
    {
        ...
        public static bool Exists(string strSql, SqlParameter[] cmdParams)
        {
            Object obj = ReturnValue(strSql, cmdParams);
            if (obj == null)
            { return false; }
            return Convert.ToInt32(obj) > 0;
        }

        public static object ReturnValue(string strSql)
        {
            return ReturnValue(strSql, null);
        }
        public static object ReturnValue(string strSql, SqlParameter[] cmdParams)
        {
        ...
        public static int ExecuteSql(string strSql, SqlParameter[] cmdParams)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
        ...
        {
            using (SqlDataAdapter da = new SqlDataAdapter(strSql, connectionString))
            {
                try
                {
        ...
        public static DataSet Query(string strSql, SqlParameter[] cmdParams)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
        ...
        public static DataSet Query(string strSql, CommandType cmdType, SqlParameter[] cmdParams)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    try
                    {
                        PrepareCommand(cmd, conn, null, cmdType, strSql, cmdParams);
                        using (SqlDataAdapter da = new SqlDataAdapter(cmd))
                        {
                            DataSet ds = new DataSet();
                            da.Fill(ds);
